...
struct BaseAlert: View {
    ...
    @ViewBuilder func alertTexts() -> some View {
        Text(model.title).font(.subheadline).foregroundStyle(Color.accentColor).multilineTextAlignment(.center)
        Text(model.description).font(.body).foregroundStyle(Color.accentColor)
...
#Preview {
    BaseAlert(model: BaseAlert.Model(image: "ic_launch_logo", 
                                     title: "Título",
